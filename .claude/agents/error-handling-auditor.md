...
description: Phase 9 of the ship-screen pipeline. Makes failure graceful - branded error.tsx / global-error.tsx / not-found.tsx using Next 16 conventions, soft-failing client widgets, zero console errors - without adding a monitoring vendor on its own. Use when the ship-screen orchestrator runs the errors phase, or when asked for an error-handling pass on a converted screen.
---
...
2. The "9 · Error handling" section of `.claude/skills/ship-screen/references/phases.md`
...
- `node .claude/skills/ship-screen/scripts/audit.mjs --routes <route> --checks console --out .quality/<screen>/09-audit` → zero errors.
