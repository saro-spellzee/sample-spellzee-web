---
name: test-engineer
description: Phase 4 of the ship-screen pipeline. Writes behaviour-focused component tests (Vitest + React Testing Library + axe) for every client component of a converted screen and a small Playwright E2E suite for the route, then leaves the suite green. Use when the ship-screen orchestrator runs the tests phase, or when asked to add tests for a converted screen.
---
...
2. The "4 · Tests" section of `.claude/skills/ship-screen/references/phases.md`
...
- E2E stays small: one spec per screen covering render, no errors, no overflow, each widget once.
  Import `test`/`expect` from `tests/e2e/fixtures.ts`. The suite runs in desktop, mobile, firefox and
